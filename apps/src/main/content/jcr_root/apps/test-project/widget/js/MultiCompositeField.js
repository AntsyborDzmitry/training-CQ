...
// TODO later END
    ...
    constructor: function(config) {
        ...
        if(!config.maxItems){
            config.fieldConfigs.maxItems = "0";
        }else{
            config.fieldConfigs.maxItems = config.maxItems;
        }
    ...
    addItem: function(name, o) {
        ...
        if(this.fieldConfigs.maxItems == 0 || this.items.getCount() <= this.fieldConfigs.maxItems){
            if (!name) {
                // new item to add
            ...
            CQ.Ext.Msg.show({
                title: 'Limit reached',
                msg: 'You are only allowed to add up to ' + this.fieldConfigs.maxItems +
                ' items to this panel.',
            ...
            if (config.orderable) {
                items.push({
                    ...
                    items: {
                        xtype: "button",
                        width: "22",
                        text: CQ.I18n.getMessage("&#8593;", null, "Ordering upwards in MultiField"),
                        handler: function(){
                ...
                items.push({
                    ...
                    items: {
                        xtype: "button",
                        width: "22",
                        text: CQ.I18n.getMessage("&#8595;", null, "Ordering downwards in MultiField"),
                        handler: function(){
                            var parent = item.ownerCt;
                            var index = parent.items.indexOf(item);
    ...
    setValue: function(value) {
        this.fields.each(function(f) {
            ...
            if (value[f.rawFieldName]) {
                ...
                // handle the raw field not matching the name being passed in
                // strip off the ./
                var fieldName = f.rawFieldName.substring(2);
                if(value[fieldName]){
                    f.setValue(value[fieldName]);
                }
            }
        });
    }
